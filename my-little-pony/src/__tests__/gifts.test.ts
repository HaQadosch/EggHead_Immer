...
describe("Toggle Reservation", () => {
  describe("on an unreserved gift", () => {
    ...
    test("should structurally share unchanged parts of the state tree", () => {
      ...
      expect(nextState.gifts["immer_licence"]).toBe(
        initialState.gifts["immer_licence"]
      );
    });
  ...
  describe("on a reserved gift", () => {
    ...
    test("should not create new gift", () => {
      expect(nextState.gifts["immer_licence"]).toEqual(
        initialState.gifts["immer_licence"]
      );
...
describe("Add book", () => {
  it("should add math book", async () => {
    ...
    expect(nextState.gifts[book.identifiers.isbn_10[0]].description).toBe(
  ...
  it("should add two books in parallel", async () => {
    ...
    const nextState = [actionBook1, actionBook2].reduce(
      giftReducer,
      createInitialState()
    );
...
describe("Patches", () => {
  describe("Reserving unreserved gift", () => {
    const initialState = createInitialState();
    const [nextState, patches, invPatches] = patchGeneratingGiftReducer(
      initialState,
      {
        type: "TOGGLE_RESERVATION",
        payload: { giftID: "egghead_subscription" }
    ...
    test("should pass the snapshot", () => {
      expect(invPatches).toMatchInlineSnapshot(`
        Array [
          Object {
            "op": "replace",
            "path": Array [
